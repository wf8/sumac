...
"""
# use BLAST to make sure all the sequences are in the same direction


# make fasta file of first sequence to use as reference
output_handle = open('subject.fasta', 'w')
SeqIO.write(records[9], output_handle, 'fasta')
output_handle.close()

for record in records:
    # compare forward and reverse of each sequence
    # see which e-Value is lower
    output_handle = open('query.fasta', 'w')
 ...
 for comparing forward and reverse with blast!!!!!
def make_distance_matrix(gb, seq_keys):
    for key in seq_keys:
        output_handle = open('subject.fasta', 'w')
        SeqIO.write(gb[key], output_handle, 'fasta')
	    ...
	    if key != key2:	        
                ...
                blastn_cmd = NcbiblastnCommandline(strand="plus", query='query.fasta', subject='subject.fasta', out='blast1.xml', outfmt=5)
                stdout, stderr = blastn_cmd()
                blastn_xml = open('blast1.xml', 'r')
                blast_records = NCBIXML.parse(blastn_xml)
		...
		output_handle = open('query.fasta', 'w')
                SeqIO.write(gb[key2].reverse_complement(), output_handle, 'fasta')
		output_handle.close()
                
		blastn_cmd = NcbiblastnCommandline(strand="plus", query='query.fasta', subject='subject.fasta', out='blast2.xml', outfmt=5)
                stdout, stderr = blastn_cmd()
                blastn_xml = open('blast2.xml', 'r')
                blast_records = NCBIXML.parse(blastn_xml)
		...
		print("key1="+key)
		print("key2="+key2)
		print("1 direction: " + str(expect1))
		print("2 direction: " + str(expect2))
